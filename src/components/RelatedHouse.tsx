      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {relatedHouses?.map((house) => (
          <Link href={`/house/${house.id}`} key={house.id}>
            <Card>
              <img src={house.thumbnail } loading='lazy' alt={house.title} width={300} height={200} className="w-full h-48 object-cover rounded-t-lg" />
              <CardContent className="p-4">
                <h3 className="font-semibold text-lg mb-2">{house.title}</h3>
                <p className="text-gray-600 mb-2">{house.location}</p>
                <p className="font-bold">${house.price}/month</p>
              </CardContent>
            </Card>
          </Link>
        ))}
